...
def process_vasp_files(base_directory):
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        # 将不同范围的数据写入到不同的Sheet
        simple_lattice_85_95.sort_values(by="lgnum").to_excel(writer, sheet_name='simple lattice (85-95)', index=False)
        simple_lattice_115_125.sort_values(by="lgnum").to_excel(writer, sheet_name='simple lattice (115-125)', index=False)

        centered_lattice_85_95.sort_values(by="lgnum").to_excel(writer, sheet_name='centered lattice (85-95)', index=False)
        centered_lattice_115_125.sort_values(by="lgnum").to_excel(writer, sheet_name='centered lattice (115-125)', index=False)

        # 统计lgnum的出现次数
        lgnum_statistics_simple = []
        lgnum_statistics_centered = []

        # 遍历所有可能的lgnum值
        for lgnum in range(8, 49):
            total_count = len(df[df["lgnum"] == lgnum])  # 计算总数
            if lgnum in centered_lattice_lgnums:
                square_count = len(centered_lattice_85_95[centered_lattice_85_95["lgnum"] == lgnum])
                hexagonal_count = len(centered_lattice_115_125[centered_lattice_115_125["lgnum"] == lgnum])
                # 将统计信息添加到列表中
                lgnum_statistics_centered.append([lgnum, square_count, hexagonal_count, total_count])
            else:
                square_count = len(simple_lattice_85_95[simple_lattice_85_95["lgnum"] == lgnum])
                hexagonal_count = len(simple_lattice_115_125[simple_lattice_115_125["lgnum"] == lgnum])
                # 将统计信息添加到列表中
                lgnum_statistics_simple.append([lgnum, square_count, hexagonal_count, total_count])

        # 创建统计信息的DataFrame
        lgnum_statistics_simple_df = pd.DataFrame(lgnum_statistics_simple, columns=["lgnum", "Square Count", "Hexagonal Count", "Total"])
        lgnum_statistics_centered_df = pd.DataFrame(lgnum_statistics_centered, columns=["lgnum", "Square Count", "Hexagonal Count", "Total"])
        # 将统计信息写入新的Sheet
        lgnum_statistics_simple_df.sort_values(by="lgnum").to_excel(writer, sheet_name='Simple lgnum Statistics',index=False)
        lgnum_statistics_centered_df.sort_values(by="lgnum").to_excel(writer, sheet_name='Centered lgnum Statistics',index=False)
