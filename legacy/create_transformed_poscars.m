function create_transformed_poscars(filename, matfile)
    % 加载.mat文件中的变换矩阵
    ...
    % 根据坐标系类型显示提示
    if strcmpi(coordinate_type, 'Direct')
        disp('读取的坐标是直角坐标系中的相对坐标 (Direct)');
    elseif strcmpi(coordinate_type, 'Cartesian')
        disp('读取的坐标是笛卡尔坐标系中的绝对坐标 (Cartesian)');
    else
        warning('未知的坐标系类型: %s', coordinate_type);
    end
    ...
    for idx = 1:length(transforms)
        ...
        % 构造输出文件名
        output_filename = fullfile(output_dir, sprintf('%s_%s.vasp', name, matrix_name));

        % 打开新的文件用于写入
        fid_out = fopen(output_filename, 'w');
        if fid_out == -1
            error('无法创建文件: %s', output_filename);
        end
        ...
        % 将原始POSCAR的第6行和第7行直接写在新文件对应行的后面
        ...
        % 写入变换后的坐标
        for i = 1:atom_count
            fprintf(fid_out, '%12.8f %12.8f %12.8f\n', transformed_coordinates(i, :));
        end
...
% 例如，create_transformed_poscars(‘POSCAR’, ‘transform_matrices.mat’);